...
async function callApi(tool, args) {
    ...
    if (!r.ok || j.ok === false) {
        throw new Error(`API ${r.status}: ${j.error || r.statusText}`);
    }
    return j.data;
}
const TOOLS = [
...
const server = new Server({ name: "sonar-crm", version: "0.1.0" }, { capabilities: { tools: {} } });
server.setRequestHandler(ListToolsRequestSchema, async () => ({ tools: TOOLS }));
server.setRequestHandler(CallToolRequestSchema, async (req) => {
    try {
        const data = await callApi(req.params.name, (req.params.arguments || {}));
        return {
