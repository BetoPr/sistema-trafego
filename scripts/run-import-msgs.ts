/**
 * Backfill: roda o import de histórico de mensagens pra um canal (usa a lib já
 * corrigida). Roda: npx tsx scripts/run-import-msgs.ts [agenciaId]
 */
...
async function main() {
  ...
  const { data: canal } = await sb
    .from("canais")
    .select("id, nome, fila_id, status, instance_token_encrypted, servidor:super_admin_servidores(base_url)")
    .eq("agencia_id", AG).eq("status", "connected")
    .order("updated_at", { ascending: false }).limit(1).maybeSingle();
  if (!canal) { console.log("Sem canal conectado pra", AG); return; }
  ...
  console.log(`Importando histórico de "${canal.nome}"...`);
  const r = await importarMensagensUazapi({
    sb, agenciaId: AG, canalId: canal.id,
    canalFilaPadrao: (canal as { fila_id?: string | null }).fila_id ?? null,
    baseUrl, token, maxChats: 80, porChat: 20,
  });
