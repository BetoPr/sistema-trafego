...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: t } = await sb
    .from("tickets")
    .select("id, numero, status, sentimento, sentimento_confianca, sentimento_motivo, resumo, resumo_atualizado_em, valor_fechado, metadata, fila_id, usuario_id, contato:contatos(id, nome, whatsapp, foto_url, ia_habilitada, email, empresa, cidade, estado, cpf), canal:canais(id, nome, status)")
    .eq("id", id)
  ...
  const [{ data: msgs }, { data: tags }] = await Promise.all([
    sb
      .from("mensagens")
      .select("id, autor, tipo, conteudo, transcricao, midia_url, midia_mime, status, created_at, usuario_id, wa_message_id, metadata, deleted_em, deleted_pra_todos, edited_em, edited_de_conteudo")
      .eq("ticket_id", id)
