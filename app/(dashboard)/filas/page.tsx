...
export default async function FilasPage({ searchParams }: PageProps) {
  ...
  const { data: filas } = await sb
    .from("filas")
    .select("id, nome, cor, descricao, ativa, ordem, created_at")
...
function labelErr(k: string) {
  return { nome_vazio: "Nome obrigatório.", db: "Erro no banco." }[k] || "Erro.";
}
...
function Field({ label, name, defaultValue, required, type = "text" }: { label: string; name: string; defaultValue?: string; required?: boolean; type?: string }) {
  return (
    <div>
      ...
      <input type={type} name={name} defaultValue={defaultValue} required={required} style={{ width: "100%", padding: type === "color" ? "2px 4px" : "8px 12px", borderRadius: 8, border: "0.5px solid var(--mk-border)", background: "var(--mk-surface-2)", color: "var(--mk-text)", fontSize: 12.5, height: type === "color" ? 36 : "auto" }} />
    </div>
  );
}
