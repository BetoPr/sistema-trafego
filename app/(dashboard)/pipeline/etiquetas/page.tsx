...
export default async function PipelineEtiquetasPage() {
  ...
  const [{ data: etqRows }, { data: contatosRows }, { data: linksRows }] = await Promise.all([
    sb.from("etiquetas").select("id, nome, cor, categoria, ativo").eq("agencia_id", ctx.agenciaId).order("nome"),
  ...
  const etiquetas = ((etqRows || []) as Array<{ id: string; nome: string; cor: string | null; categoria: string | null; ativo: boolean | null }>)
    .filter((e) => (e.categoria || "etiqueta") === "etiqueta")
    .map((e) => ({ id: e.id, nome: e.nome, cor: e.cor || "#00E19A", ativo: e.ativo !== false }));
