...
export async function atualizarPerfilAction(
  ...
  const { error } = await supabase.from("usuarios").update({ nome: parsed.data.nome }).eq("id", usuario.id);
...
export async function atualizarAgenciaAction(
  ...
  const { supabase, usuario } = await requireUserWithAgencia();
  const { error } = await supabase.from("agencias").update({ nome: parsed.data.nome }).eq("id", usuario.agencia_id);
