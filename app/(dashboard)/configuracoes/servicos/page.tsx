  ...
  return (
    <section className="mk-page">
      ...
      <div className="mk-card mk-card-lg" style={{ marginBottom: 14, display: "flex", alignItems: "center", gap: 14 }}>
        <i className={`ti ${habilitado ? "ti-toggle-right" : "ti-toggle-left"}`} style={{ fontSize: 28, color: habilitado ? "#00E19A" : "var(--mk-text-muted)" }} />
        <div style={{ flex: 1 }}>
              ...
              <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 6 }}>
                {servicos.map((s) => (
                  <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", background: "var(--mk-surface-2)", borderRadius: 8, opacity: s.ativo ? 1 : 0.55 }}>
                    <i className={`ti ${s.ativo ? "ti-check" : "ti-circle"}`} style={{ color: s.ativo ? "#00E19A" : "var(--mk-text-muted)", fontSize: 14 }} />
                    ...
                    <form action={toggleServicoAtivo}>
                      ...
                      <button type="submit" title={s.ativo ? "Pausar" : "Ativar"} style={{ background: "transparent", border: 0, color: s.ativo ? "#00E19A" : "#00E19A", cursor: "pointer", padding: "4px 8px", fontSize: 13 }}>
